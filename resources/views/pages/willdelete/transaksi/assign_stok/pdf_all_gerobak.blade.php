            ...
            <div class="row align-items-center">
              <div class="col-sm-12">
                <div class="d-flex align-items-center mb-2 navbar-brand">
                  <img src="{{ asset('logo_sruuput.webp') }}" class="img-fluid logo-lg" alt="Logo Sruuput" width="55">
                </div>
                <h5>Laporan Assign Stock / {{ tanggalIndo($tglNow) }}</h4>
              </div>
            </div>
          </div>
    </div>
        
    @foreach($data_gerobak as $dg)    
        <div class="row g-3" style="margin-bottom: 10px;">
          <div class="col-12 mb-2">
            <div class="row align-items-center g-3">
              <div class="col-sm-12">
                <h5>{{ $dg->kode }} / {{ $dg->employee->firstname . " " . $dg->employee->lastname }}</h5>
              </div>
                ...
                <tbody>
                    ...
                    @foreach ($dg->product as $pd)
                        ...
                        @if ($pd->stock->count() <= 1)
                            <tr>
                              ...
                              @if ($pd->stock->count() >= 1)
                                  ...
                                  <td>{{ $pd->stock->sum('stock') }}</td>
                                  <td>{{ $s->stock_tipe == 'Penambahan' ? '+Addon' : 'Awal' }}</td>
                                  <td>{{ Illuminate\Support\Str::limit($s->admin->firstname, 17).'..' }}</td>
                                  @endforeach
                              @else
                                  <td>0</td>
                                  <td>-</td>
                        ...
                        @else
                            ...
                            <tr>
                                <td rowspan="{{ $pd->stock->count() }}">{{ $loop->iteration }}</td>
                                <td rowspan="{{ $pd->stock->count() }}">{{ $pd->nama }}</td>
                                <td>{{ $first->stock }}</td>
                                <td>{{ $first->stock_tipe == 'Penambahan' ? '+Addon' : 'Awal' }}</td>
                                <td>{{ Illuminate\Support\Str::limit($first->admin->firstname, 17).".." }}</td>
                            </tr>

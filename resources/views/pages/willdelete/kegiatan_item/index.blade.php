...
@section('content')
    <x-page-header title="List Item" module="Management Kegiatan Item" >
        <li class="breadcrumb-item">Management</li>
        <li class="breadcrumb-item">Master Kegiatan Item</li>
    </x-page-header>
    ...
    @php
        $thead = ['No', 'Nama', 'Keterangan', 'Last Updated', 'Opsi'];
    @endphp
...
@push('js')
    ...
    <script type="text/javascript">
        ...
        let table = $('#myTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('kegiatan-item.index') }}",
            scrollY: true,
            scrollX: true,
            scrollCollapse: true,
            fixedColumns: {
        ...
        $('#form-tambah').on('submit', function(e) {
            e.preventDefault();
            $.ajax({
              url: '{{ route("kegiatan-item.simpan") }}', // Route untuk simpan data
              method: 'POST',
              data: $(this).serialize(),
        ...
        $(document).on('click', '.btn-delete', function () {
            let id = $(this).data('id');
            var url = "{{ route('kegiatan-item.hapus', ':id:') }}";

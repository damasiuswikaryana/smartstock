...
@section('content')
    <x-page-header title="Detail" module="Stock In Detail">
        ...
        @if ($data->status == 'Pending')
            <div class="col-12 d-flex justify-content-between align-items-center">
                <button type="button" id="approval-btn" class="btn btn-shadow btn-primary me-2 d-flex align-items-center">
                    <i class="ph-duotone ph-check-circle icon-search me-2"></i> Approve Stock In
                </button>
            </div>
        @else
...
@push('js')
    <script type="text/javascript">
        $(document).on('click', '#approval-btn', function() {
            let id = {{ $data->id }};
            var url = "{{ route('stockin.approve', ':id:') }}";
        ...
        $('#btnDownload').on('click', function() {
            let id = {{ $data->id }};
            let url =
                "{{ route('stockin.downloadTandaTerima', ['id' => ':id']) }}"
                .replace(':id', id);
